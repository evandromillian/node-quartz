...
function createClient(db_name, user, password, host, port, type) {
    ...
    let rpoplpush = function(list_to_pop, list_to_push, callback) {
        ...
        db.transaction(function(t) {
            return Promise.all([
                db.query('SELECT IFNULL(MIN(idx) - 1, 0) AS idx \
                            FROM key_values \
                            WHERE ky = :dest_list \
                            GROUP BY ky \
    ...
    const lrem_query_desc = 'delete from key_values where ky like :list_name order by idx desc limit :count';
    const lrem_query_asc = 'delete from key_values where ky like :list_name order by idx asc limit :count';
